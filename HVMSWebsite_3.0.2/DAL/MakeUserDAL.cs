...
namespace METAOPTION.DAL
{
    public class MakeUserDAL
    {
        ...
        {
            DALDataContext objDAL = new DALDataContext();
            DataTable dTab = new DataTable("ActiveGrps");
            using (SqlConnection Conn = new SqlConnection(objDAL.Connection.ConnectionString))
            {
                if (Conn.State == ConnectionState.Closed)
                    Conn.Open();
                ...
                //SqlCommand Cmd = new SqlCommand(sqlStatement, Conn);
                SqlCommand Cmd = new SqlCommand("ActiveGroups", Conn);
                Cmd.Parameters.AddWithValue("@GroupName", grpName);
                Cmd.Parameters.AddWithValue("@GroupDesc", grpDescription);
        ...
        {
            UpdateUserSettingsResult UII = METAOPTION.DALDataContext.Factory.DB.UpdateUserSettings(UserID, SettingKeyValue).FirstOrDefault();
        ...
        {
            List<IPUser_PermissionSelectResult> Lst = METAOPTION.DALDataContext.Factory.DB.IPUser_PermissionSelect(UserID).ToList<IPUser_PermissionSelectResult>();
            if (Lst.Count > 0)
                return Lst;
            else
                return null;
